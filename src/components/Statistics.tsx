...
const StatCard = ({ title, value, icon: Icon }: StatCardProps) => (
    <motion.div
        ...
        className="relative overflow-hidden rounded-lg border bg-background p-3 shadow-sm hover:shadow"
        whileHover={{ y: -2, transition: { duration: 0.2 } }}
    >
        <div className="relative flex items-center">
            <div className="bg-gradient-to-br from-primary to-primary/90 rounded-lg p-2 shadow-sm">
                <Icon className="h-4 w-4 text-white" />
            </div>
            <div className="ml-3 flex-1 min-w-0">
                <p className="text-xs font-medium text-muted-foreground truncate">{title}</p>
                <p className="text-lg font-bold bg-gradient-to-r from-primary to-primary/90 bg-clip-text text-transparent mt-0.5 truncate">
                    {value}
                </p>
            </div>
        </div>
