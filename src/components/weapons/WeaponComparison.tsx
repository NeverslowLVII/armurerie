    ...
    const fetchWeapons = async () => {
      try {
        ...
        toast({
          ...
          description: "Impossible de charger les données des armes. Veuillez réessayer ultérieurement.",
          duration: 5000,
        });
      } finally {
        setLoading(false);
  ...
  if (hasError && weapons.length === 0) {
    return (
      <div className="p-4 border border-red-300 bg-red-50 dark:bg-red-900/20 rounded-md text-red-800 dark:text-red-200">
        ...
        <p className="mt-1 text-sm">Impossible de charger les données des armes. Veuillez réessayer ultérieurement.</p>
      </div>
    );
  }
        ...
        <Card className="flex-1">
          ...
          <CardContent>
            <Select onValueChange={handleWeapon1Change} value={weapon1?.name || ''}>
              ...
              <SelectContent className="max-h-[300px] bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800">
                {weaponCategories.map((category) => (
                  <div key={category}>
                    <div className="px-2 py-1.5 text-sm font-semibold text-neutral-500 dark:text-neutral-400 bg-neutral-50 dark:bg-neutral-900">
                      {category}
                    </div>
                    {weapons
                      ...
                      .map((weapon) => (
                        <SelectItem 
                          key={weapon.name} 
                          value={weapon.name} 
                          className="text-neutral-800 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-800 focus:bg-neutral-100 dark:focus:bg-neutral-800"
                        >
                          {weapon.name}
                        </SelectItem>
                      ))
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="space-y-4">
          {weapon1 ? (
            ...
            <Card className="flex items-center justify-center p-6 text-neutral-500 dark:text-neutral-400 min-h-[240px]">
...
function StatRow({ label, value, compareValue, format = (v) => `${v}`, isLowerBetter = false }: StatRowProps) {
  const comparison = compareValue === undefined
    ? null
    : (isLowerBetter
        ? (value < compareValue)
        : (value > compareValue));
  ...
  return (
    <div className="flex justify-between py-1.5 border-b border-neutral-100 dark:border-neutral-700 last:border-0">
